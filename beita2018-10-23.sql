...
CREATE TABLE `enter_personnel` (
  ...
  `enter_is_check` tinyint(1) NOT NULL DEFAULT '0' COMMENT '是否已经审核',
...
CREATE TABLE `exam_news` (
  ...
  `news_isnew` tinyint(1) NOT NULL DEFAULT '0' COMMENT '是否为最新新闻',
...
CREATE TABLE `exam_signup_list` (
  ...
  `signup_isconfirm` tinyint(1) NOT NULL DEFAULT '0' COMMENT '审核是否通过',
...
CREATE TABLE `system_notice` (
  ...
  `notice_isread` tinyint(1) NOT NULL DEFAULT '0' COMMENT '是否已读',
...
CREATE TABLE `user_details` (
  ...
  `details_avatar` varchar(255) NOT NULL DEFAULT '127.0.0.1:8080/xxx/xxx' COMMENT '用户头像（有一个默认值）',
...
CREATE TABLE `user_info` (
  ...
  `userinfo_email` varchar(255) NOT NULL DEFAULT 'xxxxx@xxx.xxx' COMMENT '用户邮箱（用来通知考试，必填）',
